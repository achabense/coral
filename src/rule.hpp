...
namespace iso3 {
    ...
    inline void increase(ruleT& rule, const groupT& group) {
        assert(!group.empty());
        const cellT inc = increase(rule[group[0]]);
    ...
    // TODO: workaround for generating "sparse" rules. Rules generated by `rand_rule` are too chaotic.
    inline void randomize(ruleT& rule, std::mt19937& rand, const isotropic& iso = isotropic::get()) {
        for (const auto& group : iso.groups()) {
            if ((rand() % 16) == 0) {
                const auto v = cellT(rand() % cellT::states);
                for (const codeT c : group) {
